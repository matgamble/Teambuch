"""
WhatsApp-Icon fuer die Gruppe "Blitzer und Parkscheiben" der Community
"Die Verrueckten": ein Orka-Symbol (aus site/orka-symbol.png, Original
schwarz-weiss) auf warmem Verlaufshintergrund, mit zweizeiligem
gebogenem Schriftzug "Blitzer und" / "Parkscheiben" (Georgia italic,
wie das "Teambuch"-Wortlogo), "Kita Reischlestraße" unten und duennem
Gold-Ring als Rahmen. Gleiches Layout wie build.py ("Die Verrueckten").

Aufruf: python3 build_blitzer.py -> erzeugt icon-blitzer-parkscheiben.png
...
paste_y = int(SIZE * 0.605) - target_h // 2
...
# zweizeiliger Bogen-Schriftzug "Blitzer und" / "Parkscheiben"
arctext = Image.open(BASE / "arctext-blitzer-parkscheiben.png").convert("RGBA")
at_w = int(SIZE * 0.78)
...
final.save(OUT_DIR / "icon-blitzer-parkscheiben.png")
